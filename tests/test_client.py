...
def test_fetch_customer_usage_params_match_discovery_schema():
    ...
    c.fetch_customer_usage(date="2026-08-30", parameters="gmail:num_emails_sent")
    sent = dict(calls[0])
    sent["customerId"] = "C0123"  # discovery pattern is ``C.+|my_customer``
    sent.pop("pageToken", None)  # first page passes pageToken=None

    real = build("admin", "reports_v1", static_discovery=True, developerKey="unused")
    real.customerUsageReports().get(**sent)  # raises TypeError on an unknown/invalid kwarg
