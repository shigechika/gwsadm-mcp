#!/usr/bin/env python3
"""Exercise every registered tool against the live Workspace and report what is broken.
...
never payloads, which would otherwise carry account and document names into
logs.

Usage:
    # In-process: imports the server, uses its configured service accounts.
    GWSADM_CONFIG=... uv run python scripts/smoke_test.py
...
async def main_async(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        results = await run_probes(
            ...
            # account addresses and document titles throughout, and an error
...
def main() -> None:
    ...
    # Serial by default: the Reports API is quota-limited per tenant and
